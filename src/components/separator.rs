pub fn render(ui: &mut egui::Ui, ratio: f32) {
    ...
    if is_vertical {
        ...
        let (rect, _) = ui.allocate_exact_size(
            egui::vec2(total_width, stroke.width.max(1.0)),
            egui::Sense::hover(),
        );
        ...
        ui.painter().line_segment(
            [egui::pos2(x_start, y), egui::pos2(x_start + line_width, y)],
            stroke,
        );
    } else {
        ...
        let (rect, _) = ui.allocate_exact_size(
            egui::vec2(stroke.width.max(1.0), total_height),
            egui::Sense::hover(),
        );
        ...
        let x = rect.center().x;
        ui.painter().line_segment(
            [egui::pos2(x, y_start), egui::pos2(x, y_start + line_height)],
            stroke,
        );
    }
}
